...
public class RouterC
{
    ...
    RouterC() throws SocketException, UnknownHostException {
        Initialise();
    ...
    {
        this.address = new Address(InetAddress.getByName("localhost"),5002);
        this.socket = new DatagramSocket(this.address.port);   //takes current ip by default

        //setting neighbours
        neighbours = new ArrayList<>();
        Neighbour temp1 = new Neighbour(InetAddress.getByName("localhost"),5000,2);
        Neighbour temp2 = new Neighbour(InetAddress.getByName("localhost"),5001,1);
    ...
    public static void main(String[] args) throws IOException {
        RouterC routerC = new RouterC();

        while (routerC.receivedLSP.size()!=routerC.NUM_NODES)
        {

            //receive
            int numToReceive = (int) routerC.NumToReceive(0);
            System.out.println("Received: "+numToReceive);
            routerC.Receive(numToReceive);

            //receive
            numToReceive = (int) routerC.NumToReceive(1);
            System.out.println("Received: "+numToReceive);
            routerC.Receive(numToReceive);

            //prepare the LSPs to be sent
            ArrayList<LSPRecord> listToSend = new ArrayList<>();
            for(int i=0;i<routerC.receivedLSP.size();i++)
            {
                if(routerC.receivedLSP.get(i).sent == 0)
                {
                    LSPRecord temp = routerC.receivedLSP.get(i);
                    listToSend.add(temp);
                    temp.sent=1;
                    routerC.receivedLSP.set(i,temp);
                }
            }
            ...
            //send LSPs
            routerC.SendNumLSP(listToSend.size());
            for (int i=0;i<listToSend.size();i++)  //for each LSP
            {
                for(int j=0;j<routerC.neighbours.size();j++)  //for each neighbour
                {
                    routerC.Send(new Address(routerC.neighbours.get(j).neighbourAddr,routerC.neighbours.get(j).neighbourPort),listToSend.get(i).lsp);
                }
            }

//            numToReceive = (int) routerA.NumToReceive(1);
//            routerA.Receive(numToReceive);
        ...
        for (int i=0;i<routerC.receivedLSP.size();i++)
        {
            System.out.println(routerC.receivedLSP.get(i));
        }
