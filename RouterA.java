...
public class RouterA
{
    ...
    RouterA() throws SocketException, UnknownHostException {
        Initialise();
    ...
    {
        this.address = new Address(InetAddress.getByName("localhost"),5000);
        this.socket = new DatagramSocket(this.address.port);   //takes current ip by default

        //setting neighbours
        neighbours = new ArrayList<>();
        Neighbour temp1 = new Neighbour(InetAddress.getByName("localhost"),5001,4);
        Neighbour temp2 = new Neighbour(InetAddress.getByName("localhost"),5002,2);
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (routerA.receivedLSP.size()!=routerA.NUM_NODES)
        {
            //prepare the LSPs to be sent
            ArrayList<LSPRecord> listToSend = new ArrayList<>();
            for(int i=0;i<routerA.receivedLSP.size();i++)
            {
                if(routerA.receivedLSP.get(i).sent == 0)
                {
                    LSPRecord temp = routerA.receivedLSP.get(i);
                    listToSend.add(temp);
                    temp.sent=1;
                    routerA.receivedLSP.set(i,temp);
                }
            }
            ...
            //send LSPs
            routerA.SendNumLSP(listToSend.size());
            for (int i=0;i<listToSend.size();i++)  //for each LSP
            {
                for(int j=0;j<routerA.neighbours.size();j++)  //for each neighbour
                {
                    routerA.Send(new Address(routerA.neighbours.get(j).neighbourAddr,routerA.neighbours.get(j).neighbourPort),listToSend.get(i).lsp);
                }
            }

            //receive
            int numToReceive = (int) routerA.NumToReceive(0);
        ...
        for (int i=0;i<routerA.receivedLSP.size();i++)
        {
            System.out.println(routerA.receivedLSP.get(i));
        }
